#encargado de modificar el estado de nuestra representaicon 
module Actions
    def self.move_snake(state)
      next_direction = state.curr_direction
      next_position = calc_next_position(state)#hacemos referencia a un metodo de instancia(en los modulos no se puede a menos que sean de clas con self.)
      ...
      #si no es valida termina el juego si no la serpiente se mueve
    ...
    def self.move_snake_to(state, next_position)
        #simula la ultima posicion de la serpiente y rota a la nueva
      new_positions = [next_position] + state.snake.positions[0...-1]#rango no inclusivo
      state.snake.positions = new_positions
      state
    end
